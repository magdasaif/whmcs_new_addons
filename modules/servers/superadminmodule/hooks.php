 ...
 */
function hook_superadminmodule_clientedit(array $params)
{
    try {
 ...
 */
add_hook('ClientEdit', 1, 'hook_superadminmodule_clientedit');
...
add_hook('ClientAreaPrimaryNavbar', 1, function ($menu)
{
    ...
    if (!is_null($menu->getChild('Services'))) {
        ...
        $menu->getChild('Services')
            ->addChild(
                'Superadmin Module Products',
                array(
                    'uri' => 'clientarea.php?action=services&module=superadminmodule',
                    'order' => 15,
                )
            );
    }
...
add_hook('ClientAreaSecondarySidebar', 1, function ($secondarySidebar)
{
    ...
    if (!is_null($secondarySidebar->getChild('My Services Actions'))) {
        ...
        // $customPanel = $secondarySidebar->addChild('Dashboard Details for Services');
        ...
        // $services=Capsule::table('tblhosting')->where('userid',$user->id)->get();
        // foreach ($services as $service) {
        //     $package_name=Capsule::table('tblproducts')->where('id',$service->packageid)->first()->name;

        //     $customPanel->addChild(
        //         $package_name ,//.'('.$service->domain.')',
        //         array(
        //             'uri' => 'clientarea.php?action=productextradetails&module=superadminmodule&id='.$service->id,
        //             // 'uri' =>'https://whmcs.murabba.dev/clientarea.php?action=productdetails&module=superadminmodule&new=1&id='.$service->id,
        //             'icon'  => 'fa-list-alt',
        //             'order' => 1,
        //         )
        //     );
