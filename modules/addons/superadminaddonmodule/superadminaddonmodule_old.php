...
function superadminaddonmodule_activate()
{
    ...
    try {
        ...
        return [
            // Supported values here include: success, error or info
            'status' => "error",
            'description' => 'Unable to create mod_superadminaddon: ' . $e->getMessage(),
        ];
    }
}
...
function superadminaddonmodule_deactivate()
{
    ...
    try {
        Capsule::schema()
            ->dropIfExists('mod_superadminaddon');
        ...
        return [
            // Supported values here include: success, error or info
            "status" => "error",
            "description" => "Unable to drop mod_superadminaddon: {$e->getMessage()}",
        ];
    }
}
...
function superadminaddonmodule_upgrade($vars)
{
    ...
    if ($currentlyInstalledVersion < 1.1) {
        ...
        $schema->table('mod_superadminaddon', function($table) {
            $table->text('demo2');
        });
    }
