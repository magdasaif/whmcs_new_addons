            ...
            <div class="card bg-default">
                ...
                <div class="table-responsive">
                    <table class="table table-sm">
                        ...
                        <tr>
                            {*
                            <td><strong>{lang key='tenant'}</strong></td>
                            <td><strong>{lang key='package'}</strong></td>
                            <td><strong>{lang key='duration'}</strong></td>
                            <td><strong>{lang key='type'}</strong></td>
                            *}
                            <td><strong>{lang key='Description'}</strong></td>
                            <td width="20%" class="text-center"><strong>{lang key='invoicesamount'}</strong></td>
                        </tr>
                        </thead>
                        <tbody>
                        {foreach $invoiceitems as $item}
                        {assign var="all_descriptions" value=explode(",", $item.description)}
                            <tr>
                                <td>{$item.description}{if $item.taxed eq "true"} *{/if}</td>
                                
                                {*
                                <td class="text-center">{$all_descriptions[0]}</td>
                                <td class="text-center">{$all_descriptions[1]}</td>
                                <td class="text-center">{$all_descriptions[2]}</td>
                                <td class="text-center">{$all_descriptions[3]}</td>
                                *}
